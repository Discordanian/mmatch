...
BEGIN
    ...
    /* The next line is to only update the password if a new password was specified */
       UPDATE app_user SET pwhash = p_pwhash WHERE user_id = v_user_id AND p_pwhash IS NOT NULL;
       /* The next two lines are to set the email to unverified if the email address is changed */ 
       UPDATE app_user SET email_unverified = p_email WHERE user_id = v_user_id AND email_verified IS NOT NULL AND email_verified != p_email; 
       UPDATE app_user SET email_unverified = p_email WHERE user_id = v_user_id AND email_verified IS NULL AND email_unverified != p_email; 
END
            $$
DELIMITER ;
